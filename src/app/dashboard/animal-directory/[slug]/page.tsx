...
export default function AnimalDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    if (!isNewAnimal) {
      ...
      const mockAnimal = {
        ...
        media: {
          images: [
            { id: "img1", name: "Tiger 1", url: "/placeholder.svg?height=400&width=600", type: "image/jpeg" },
            { id: "img2", name: "Tiger 2", url: "/placeholder.svg?height=400&width=600", type: "image/jpeg" },
          ],
          videos: [
            { id: "vid1", name: "Tiger in action", url: "/placeholder.svg?height=400&width=600", type: "video/mp4" },
          ],
          arModels: [
            {
              id: "ar1",
  ...
  const handleSave = async () => {
    try {
      ...
      toast({
        ...
        description: `${animal.name} has been ${isNewAnimal ? "added to" : "updated in"} the directory.`,
      })

      if (isNewAnimal) {
        // Redirect to the new animal page
        router.push(`/dashboard/animal-directory/${animal.name.toLowerCase().replace(/\s+/g, "-")}`)
      } else {
        setIsEditing(false)
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push("/dashboard/animal-directory")
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleFileUpload = (files: File[], mediaType: "images" | "videos" | "arModels") => {
  ...
  const removeFile = (fileId: string, mediaType: "images" | "videos" | "arModels") => {
    setAnimal((prev) => ({
      ...prev,
      media: {
        ...prev.media,
        [mediaType]: prev.media[mediaType].filter((file: any) => file.id !== fileId),
      },
    }))
  }
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <Button variant="outline" size="icon" onClick={() => router.push("/dashboard/animal-directory")}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <h2 className="text-3xl font-bold tracking-tight">{isNewAnimal ? "Add New Animal" : animal.name}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-2">
                      <div>
                        <span className="font-medium">Scientific Name:</span> {animal.scientificName}
                      </div>
                      <div>
                        <span className="font-medium">Category:</span> {animal.category}
                      </div>
                      <div>
                        <span className="font-medium">Conservation Status:</span> {animal.conservationStatus}
                      </div>
                    </div>

                    <h3 className="text-lg font-semibold mt-4 mb-2">Habitat & Diet</h3>
                    <div className="space-y-2">
                      <div>
                      ...
                      <div>
                        <span className="font-medium">Lifespan:</span> {animal.lifespan}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-semibold mt-4 mb-2">Available at</h3>
                      ...
                      {animal.zooIds.map((zooId: string) => {
                        ...
                        return zoo ? (
                          <Badge
                            ...
                            onClick={() => router.push(`/dashboard/zoo-profiles/${zooId}`)}
                          >
                            {zoo.name}
                          </Badge>
                        ) : null
                ...
                {animal.media.videos.map((video: any) => (
                  <div key={video.id} className="relative group">
                    <div className="aspect-video relative overflow-hidden rounded-md border bg-gray-100 flex items-center justify-center">
                      ...
                      <Image src={video.url || "/placeholder.svg"} alt={video.name} fill className="object-cover" />
                    </div>
                    <Button
                      variant="destructive"
                      size="icon"
                ...
                {animal.media.arModels.map((model: any) => (
                  <div key={model.id} className="relative group">
                    <div className="aspect-square relative overflow-hidden rounded-md border bg-gray-100 flex items-center justify-center">
                      ...
                      <Image src={model.url || "/placeholder.svg"} alt={model.name} fill className="object-cover" />
